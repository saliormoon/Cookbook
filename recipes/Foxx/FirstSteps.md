...
**Note:** For this recipe you need at least Arango 2.4. For versions before look at the [old My first Foxx App](FirstStepsLegacy.md).
...
If you want to learn more about `db`, please check out [our documentation about handling collections](https://docs.arangodb.com/2.8/Collections/index.html). The setup script will be executed when the app is installed. In the development mode however, it will be called every time we request the app. If we click on the tiny `i` on our app, we will get to interactive documentation for our app (which is empty right now, because we don't have any routes yet). This will trigger our app and therefore execute our setup script. To check if that worked, go to `collections`. You will see a collection called `dev_todos_todos`. Setup is done!
...
If you go to the interactive documentation again, you will now see the description you just added. Great! If you want more information about controllers, see [in the documentation](https://docs.arangodb.com/2.8/Foxx/Develop/Controller.html).

### Add a model that describes our todo items

We now need to define how we want a single todo item looks like. Foxx uses this information for both the documentation as well as for validating inputs. In a file called `todo.js` in the folder `models` you put the following [Foxx Model](https://docs.arangodb.com/2.8/Foxx/Develop/Model.html) prototype:
...
* If you want to have custom methods on your repository, you can extend it in the same way you extended the Foxx.Model. [Learn more about it here](https://docs.arangodb.com/2.8/Foxx/Develop/Repository.html)
...
* We will also talk about workers and how to do work outside the request-response-cycle. Until then [check out the documentation for it](https://docs.arangodb.com/2.8/Foxx/Develop/Queries.html).
